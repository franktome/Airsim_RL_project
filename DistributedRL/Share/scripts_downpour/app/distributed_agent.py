...
class DistributedAgent():
    ...
    def __connect_to_airsim(self):
        ...
        while True:
            try:
                ...
                self.__car_client = airsim.CarClient(ip="127.0.0.1", port=41451)
                self.__car_client.confirmConnection()
                self.__car_client.enableApiControl(True)
                ...
                if (attempt_count % 10 == 0):
                    ...
                    if self.__local_run:
                        ...
                        os.system(f"xvfb-run --server-num=98 -s '-screen 0 1024x768x24' {os.path.join(self.__airsim_path, 'Blocks.sh')} &")
                    else:
    ...
    # quaternion() 함수 추가
    def __to_quaternion(self, pitch, roll, yaw):
        cy = math.cos(yaw * 0.5)
        sy = math.sin(yaw * 0.5)
    ...
    def __run_airsim_epoch(self, always_random):
        ...
        #print("s_point:", starting_points)
        #print("s_direction:", starting_direction)
        #starting_points = [-5.435961725197558e-07, -1.219881028191594e-06, 0.23386596143245697]
        #starting_direction = (0.0, 0.0, 0.0)
        ...
        print('Getting Pose')
        pose = airsim.Pose(
            airsim.Vector3r(starting_points[0], starting_points[1], starting_points[2]),
            self.__to_quaternion(starting_direction[0], starting_direction[1], starting_direction[2])
            #airsim.Quaternionr(7.294742681551725e-05, -5.36441632448259e-07, 1.5567880318201333e-09, 1.0)
        )
        self.__car_client.simSetVehiclePose(pose, True)
    ...
    # Gets an iDistributedRL/Templatemage from AirSim
    def __get_image(self):
        ...
        channel_count = int(len(image1d) / (image_response.height * image_response.width))
        image_rgba = image1d.reshape(image_response.height, image_response.width, channel_count)
    ...
    def __compute_reward(self, collision_info, car_state):
        ...
        position = self.__car_client.simGetVehiclePose().position
        print("Current position:", position)
        car_point = np.array([position.x_val, position.y_val, 0])
